...
Выдать отчет в формате из `agent.md`. Если данных недостаточно, не заполнять пробелы выдумками: явно назвать missing data, уровень уверенности и следующий шаг.
...
- Output: финальный ответ в формате `agent.md` или короткий формат, если пользователь явно попросил кратко.
